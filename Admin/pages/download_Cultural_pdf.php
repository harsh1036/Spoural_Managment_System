...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $pdf->SetAuthor('Admin');
    $pdf->SetTitle('Cultural Event Participants Report');
    ...
    if ($download_all && $dept_id) {
        ...
        // Get all Cultural events for this department
        $eventQuery = "SELECT DISTINCT e.id, e.event_name 
                      FROM events e 
                      JOIN participants p ON e.id = p.event_id 
                      WHERE e.event_type = 'Cultural' AND p.dept_id = :dept_id 
    ...
    // Output the PDF
    $pdf->Output('Cultural_event_participants_report.pdf', 'D');
    exit();
}
