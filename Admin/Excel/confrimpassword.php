...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['email'])) {
    ...
    // Get the part before @charusat.edu.in from email
    $email_id = str_replace('@charusat.edu.in', '', $email);
    ...
    try {
        ...
        // First get all admin_ids from the admins table
        $check_sql = "SELECT admin_id FROM admins";
        $result = $db->query($check_sql);
        
        $found = false;
        if ($result) {
            while ($row = $result->fetch_assoc()) {
                if ($row['admin_id'] === $email_id) {
                    $found = true;
        ...
        if (!$found) {
            throw new Exception("No matching admin ID found for: $email_id");
        }
        ...
        if(!$uppercase || !$lowercase || !$number || !$specialChars || strlen($new_password) < 8) {
            throw new Exception('Password should be at least 8 characters long and should include at least one uppercase letter, one lowercase letter, one number, and one special character.');
        }
        else if ($new_password !== $confirm_password) {
            throw new Exception('Passwords do not match. Please try again.');
        }
        else {
            // Update password for the matched admin_id
...
<body>
    <div class="container">
        <div class="logo">
            <img src="../assets/images/logo.png" alt="CHARUSAT Logo">
        </div>
